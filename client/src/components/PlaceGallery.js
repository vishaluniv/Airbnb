            ...
            <div className="absolute inset-0 bg-black bg-opacity-30 min-h-screen">
                <div className="bg-black p-8 grid gap-4">
                    ...
                    {place.photos.length > 0 && place.photos.map(place => (
                            <img className="w-screen object-cover" src={`https://airbnb-hd8f.onrender.com/uploads/${place}`} alt=""/>
                        ))}
                </div>
            </div>
        );
    ...
    return(
        <div className="relative">
            <div className="grid gap-2 grid-cols-[2fr_1fr] my-6 rounded-3xl overflow-hidden">
                {place.photos?.[0] && <img onClick={()=> setShowAllPhotos(true)} className="aspect-square cursor-pointer object-cover" src={`https://airbnb-hd8f.onrender.com/uploads/${place.photos[0]}`} alt={place.title}/>}
                <div className="grid">
                    {place.photos?.[1] && <img onClick={()=> setShowAllPhotos(true)} className="aspect-square cursor-pointer object-cover" src={`https://airbnb-hd8f.onrender.com/uploads/${place.photos[1]}`} alt={place.title}/>}
                    <div className="overflow-hidden">
                        {place.photos?.[2] && <img onClick={()=> setShowAllPhotos(true)} className="aspect-square cursor-pointer object-cover relative top-2 " src={`https://airbnb-hd8f.onrender.com/uploads/${place.photos[2]}`} alt={place.title}/>}
                    </div>
                </div>
            </div>
